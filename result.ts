 ...
 */
export class ErrImpl<E> implements BaseResult<E, E> {
  /** An empty Err */
  ...
  toString(): string {
    return `Err(${Utils.toString(this.val)})`;
  }
...
export class OkImpl<T> implements BaseResult<T, never> {
  ...
  toString(): string {
    return `Ok(${Utils.toString(this.val)})`;
  }
}
...
export namespace Result {
  ...
  export function all<T extends Result<any, any>[]>(
    ...
    const okResult: ResultOkTypes<T>[] = [];
  ...
  export function any<T extends Result<any, any>[]>(
    ...
    const errResult: ResultErrTypes<T>[] = [];
